...
for (const [team, odd] of Object.entries(game.odds)) {
  const outPut = team === "x" ? "draw" : `vicotry ${game[team]}`;
  console.log(team);
...
const newGameEvent = [...gameEvents.entries()];

for (const [time, event] of newGameEvent) {
  const output = time <= 45 ? "[FIRST HALF]" : "[SECOND HALF]";
  console.log(`${output} ${time}: ${event}`);
}
